...
| 群友榜 | 同时按 **发送者** 汇总：战报图与插件页面都展示「群友榜 · 今日验车次数」，谁验车最多一目了然 |
| 摆烂图 | 当天**一辆车都没**时，定时推送会**直接发出你指定的图片**（默认内置 `resources/empty.jpg`：蓝发鲸鱼娘 +「今天一辆车都没有，群友真是不行呢」），也可切成"现场生成摆烂图"（睡觉猫 + 文案） |
...
| `push_empty_report` | bool | `true` | 当天无记录是否也推送「摆烂图」 |
| `empty_report_mode` | string | `image` | `image` = 直接发送指定图片；`generated` = 现场生成摆烂图（睡觉猫 + 文案） |
| `empty_report_image` | string | `""` | 空战报图片路径；留空用内置 `resources/empty.jpg`，也可把 `empty.jpg` / `lazy.jpg` 放进插件数据目录覆盖 |
| `empty_report_text` | string | `今天一辆车都没，真是怠惰呢` | 随图文字（生成模式下也显示在图上）；**留空 = 只发图片不带文字** |
| `push_show_senders` | bool | `true` | 战报图是否显示「群友榜」（发送者 + 各自验车次数） |
| `push_sender_top_n` | int | `5` | 群友榜显示人数上限（0 = 不显示） |
...
* **推送没动静，怎么排查？** 在群里（群主/群管理员）发一条 **`#验车状态`**，它会直接告诉你：
  ...
  2. **时区**：`push_time` 默认按 **AstrBot 所在机器的本地时间**解释。VPS 多为 UTC，此时填 `23:00`
     实际会在北京时间次日 07:00 推送。两种解决办法：
     * 推荐：把 `push_timezone` 填成 `Asia/Shanghai`（或零依赖的 `+08:00`），`push_time` 保持 `23:00`；
     * 或留空时区，自行换算时间：**北京时间 23:00 = UTC 15:00**（UTC+8），即 `push_time` 填 `15:00`。
     自检信息第一行会显示服务器时区偏移（如 `UTC+00:00`），据此判断。
