...
* **推送收不到**
  1. 确认机器人当时在线（定时任务在插件 `initialize()` 中启动，AstrBot 重启后会自动恢复）；
  2. 白名单非空时只会推送给白名单内的群；白名单为空则只推送给「当天有记录」的群；
  3. `push_enabled` 是否为 `true`，`push_time` 是否已过；`push_empty_report=false` 时当天无记录不会推送；
  4. 群号必须能从历史消息或平台群列表定位到会话（机器人需在群里且至少见过一次该群的消息，或该群在机器人 `get_group_list` 中）；
  5. 推送时间使用 **AstrBot 所在机器的本地时间**，注意服务器时区。
