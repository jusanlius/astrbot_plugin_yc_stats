...
| 摆烂图 | 当天**一辆车都没**时，定时推送照常发图：睡觉猫 +「今天一辆车都没，真是怠惰呢」（文案可配置、可关闭） |
...
| `push_empty_report` | bool | `true` | 当天无记录是否也推送「摆烂图」（睡觉猫 + 怠惰文案） |
| `empty_report_text` | string | `今天一辆车都没，真是怠惰呢` | 空战报图上的主文案 |
